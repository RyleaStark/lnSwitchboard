...
lnSwitchboard validates the token against the explicit Cloudflare account ID and existing tunnel ID you supply, encrypts the API token at rest, and never returns it to the browser. Select an authorized zone and enter an exact zone apex or subdomain. Multiple hostnames—including multiple names in one zone—can share the connection. Existing DNS is adopted only when the exact CNAME already targets the selected tunnel; records targeting another tunnel are refused and never rewritten.

The page remains visible but disabled until the connector is installed. lnSwitchboard never creates or deletes the selected tunnel. For each hostname it creates or validates DNS before changing ingress, then publishes exactly the LNURL-pay and NIP-05 paths to the isolated `21212` public listener. Pre-existing exact managed-path routes are refused because Cloudflare ingress entries have no ownership marker. Managed paths are ordered before broader hostname rules, while unrelated ingress and the terminal fallback retain their relative order. Every whole-configuration write is read back and compared with the complete intended document. You can remove one hostname independently; full disconnect removes every managed hostname route and the shared connector token. App-created DNS is deleted only after immediate ownership revalidation, while adopted or operator-changed records are preserved.
